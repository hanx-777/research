...
class TranscoderReplacementContext:
    def __init__(self,configs, model, transcoders):
        self.configs = configs
        self.layers = [configs.target_layer for t in transcoders]
    ...
    def __enter__(self):
        for transcoder in self.transcoders:
           self.model.blocks[self.configs.target_layer].mlp = TranscoderWrapper(transcoder)
