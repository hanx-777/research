...
class ActivationsStore:
    ...
    """
    def __init__(
        self, cfg,   model: HookedTransformer, create_dataloader: bool = True, dataset=None, tokenizer=None):
        self.cfg = cfg
        self.model = model
        self.dtype = torch.float32 # cfg.dtype
    ...
    def get_batch_tokens(self):
        ...
        while batch_tokens.shape[0] < batch_size:
            if not self.cfg.is_dataset_tokenized:
                ...
                try:
                    token = next(self.iterable_dataset)["tokens"]
            ...
            if self.cfg.from_pretrained:
                bos_token_id_tensor = torch.tensor([self.model.tokenizer.bos_token_id], device=tokens.device, dtype=torch.long)
            else:
                bos_token_id_tensor = torch.tensor([self.tokenizer.bos_token_id], device=tokens.device, dtype=torch.long)
    ...
    def get_activations(self, batch_tokens, get_loss=False):
        ...
        action_lists_in = []
        action_lists_out = []
        # for l in range(self.cfg.max_layer):
    ...
    def get_buffer(self, n_batches_in_buffer):
        ...
        if self.cfg.use_cached_activations:
            ...
            new_buffer = torch.zeros((buffer_size, d_in), dtype=self.dtype,# self.cfg.dtype,
                                     device=self.cfg.act_store_device)
            n_tokens_filled = 0
        ...
        new_buffer = torch.zeros(
            ...
            dtype=self.dtype,# self.cfg.dtype,
            device=self.cfg.act_store_device,
        )
